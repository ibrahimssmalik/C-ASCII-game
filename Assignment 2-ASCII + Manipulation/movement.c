...
{	
	...
	if( option == 'w' && *playDire == 'u' )   /* in the case the option selected is to move up (i.e. 'w') and the player direction is already upwards */
	{
		if( ( **pr - 1 != 0 ) && ( array[**pr-1][**pc] != ( '\\' ) ) && ( array[**pr-1][**pc] != ( '/' ) ) )   /* in the case the player row is not 0 (i.e. not equal to play map boundary) and there is no mirror above the player */
		{
			...
			**pr = **pr - 1;   /* updates the player row */
			check = lose( array, row, col, **pr, **pc, er, ec, *enemDire, outPtr, arr, arrOut );   /* check whether the player has died by moving in front of the enemy and passes output into 'check' (0 if still alive and 1 if died) */
			if( check != 1 )   /* in the case the player is still alive */
			{
				check2 = kill( array, row, col, **pr, **pc, *playDire, er, ec, outPtr, arr, arrOut );   /* check whether the player is directly around the enemy in which case the player wins and game ends (0 if still alive and 1 if died) */
			}
		}
	}
	else if( option == 'w' && *playDire != 'u' )   /* in the case the option selected is to move up (i.e. 'w') and the player direction is not upwards */
	{
	...
	{
		if( ( **pr + 1 != row ) && ( array[**pr+1][**pc] != ( '\\' ) ) && ( array[**pr+1][**pc] != ( '/' ) ) )
		{
			array[**pr+1][**pc] = 'v';
			array[**pr][**pc] = ' ';
			**pr = **pr + 1;
			check = lose( array, row, col, **pr, **pc, er, ec, *enemDire, outPtr, arr, arrOut );
			if( check != 1 )
			{
				check2 = kill( array, row, col, **pr, **pc, *playDire, er, ec, outPtr, arr, arrOut );
			}
		}
	}
	else if( option == 's' && *playDire != 'd' )
	...
	{
		if( ( **pc - 1 != 0 ) && ( array[**pr][**pc-1] != ( '\\' ) ) && ( array[**pr][**pc-1] !=  ( '/' ) ) )
		{
			array[**pr][**pc-1] = '<';
			array[**pr][**pc] = ' ';
			**pc = **pc - 1;
	...
	{
		if( ( **pc + 1 != col ) && ( array[**pr][**pc+1] != ( '\\' ) ) && ( array[**pr][**pc+1] != ( '/' ) ) )
		{
			array[**pr][**pc+1] = '>';
			array[**pr][**pc] = ' ';
			**pc = **pc + 1;
...
int lose( int **array, int row, int col, int pr, int pc, int er, int ec, char ed, void *outPtr, map *arr, LinkedList *arrOut )
{
	...
	if( ed == 'l' && er == pr && ec > pc )   /* in the case the enemy direction is left-aligned and the enemy and player are in the same row and the enemy is towards the right of the player (i.e. has a higher column number) */
	{
		for( ec = ec; ec > pc+1 ; ec-- )   /* keep incrementing the enemy column value until it reaches the second-last value (i.e. just before player ) */
		{
	...
	else if( ed == 'r' && er == pr && ec < pc )
	{
		for( ec = ec; ec < pc-1; ec++ )
		{
			...
			copyArray( array, row, col, arr, arrOut );
	...
	return check;   /* 'check' output of the function (0 if game still running and 1 if game has ended (i.e. player has lost) */
}
...
int kill( int **array, int row, int col, int pr, int pc, char pd, int er, int ec, void *outPtr, map *arr, LinkedList *arrOut )
{
	...
	{
		win( array, row, col, er, ec, outPtr, arr, arrOut );   /* initializes the 'win()' function in 'functions.c' to win the game */
	...
	{
		win( array, row, col, er, ec, outPtr, arr, arrOut );
		check = 1;
	}
	...
	return check;   /* 'check' is output of the function (0 if game still running and 1 if game has ended (i.e. player has won) */
}
